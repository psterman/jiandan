...
class ImageCrawler:
    ...
    def download_image(self, url):
        ...
        try:
            ...
            if 'jpeg' not in content_type and 'jpg' not in content_type:
                logging.info(f'跳过非jpg图片: {url} (类型: {content_type})')
                return False
            ...
            if response.status_code == 200:
                ...
                self.image_count += 1
                logging.info(f'成功下载: {save_path} ({file_size/1024:.1f}KB)')
                return True
    ...
    def crawl(self):
        ...
        try:
            ...
            for a in nav:
                href = a.get('href', '')
                if match := re.search(r'/pic/page-(\d+)', href):
                    ...
                    for post in posts:
                        ...
                        if self.is_yesterday_post(post_time):
                            ...
                            for img_hash in img_hashes:
                                try:
                                    img_url = b64decode(img_hash.get_text().strip()).decode()
